# Global settings and functions for METEOR Shiny app
...
# Function to retrieve the data from the meteor environment
get_app_data <- function() {
  meteor_env <- getOption("meteor.env")
  if (is.null(meteor_env) || is.null(meteor_env$data)) {
    return(NULL)
  }
  return(meteor_env$data)
}
